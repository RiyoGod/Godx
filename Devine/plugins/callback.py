...
from Devine import bot, MODULE, INFO as GET_INFO
...
async def help_back(_, query):
   user_id = (await GET_INFO.devine()).id
...
async def help_commnds(_, query):
   ...
   help = data[0]['help']
   button = InlineKeyboardMarkup([[InlineKeyboardButton("BACK",callback_data="help_back")]])
